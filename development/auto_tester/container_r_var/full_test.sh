...
/opt/R/3.6/bin/Rscript    -e "$R_OPTS remotes::install_local('CoRC.tar.gz', INSTALL_opts = '--install-tests')"
...
/opt/R/3.6/bin/Rscript    -e "CoRC::getCopasi(path = 'COPASI.so')"
...
/opt/R/3.6/bin/Rscript    -e "sessioninfo::session_info('CoRC')"
...
/opt/R/3.6/bin/Rscript    -e 'R.version.string; library(methods); library(isnullptr); dyn.load("COPASI.so"); source("swig_wrapper.R"); m <- CRootContainer_addDatamodel(); m$loadModel("test_names.cps"); m$getModel()$getObjectName()'
...
/opt/R/3.6/bin/Rscript    -e "R.version.string; testthat::test_package('CoRC')"
