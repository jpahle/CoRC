...
```{bash, eval=FALSE}
sudo apt install build-essentials gawk wget astyle
```

Consider using linuxbrew to install other dependencies:
...
cmake -DBUILD_GUI=OFF -DBUILD_SE=OFF -DENABLE_R=ON -DR_USE_DYNAMIC_LOOKUP=ON -DENABLE_NEW_PARSER=OFF -DCOPASI_DEPENDENCY_DIR=../copasi-dependencies/bin ../COPASI
make
cd ../../../
```
...
to update Copasi Version, use git bash for example:
...
cmake -G Ninja -DCMAKE_BUILD_TYPE=Release -DCMAKE_C_COMPILER=cl -DCMAKE_CXX_COMPILER=cl -DBUILD_GUI=OFF -DBUILD_SE=OFF -DENABLE_R=ON -DENABLE_NEW_PARSER=OFF -DCOPASI_DEPENDENCY_DIR="../copasi-dependencies/bin_vs15_x64_Release" -DSWIG_EXECUTABLE="../swigwin/swig.exe" -DR_INCLUDE_DIRS="C:/Program Files/R/R-3.4.1/include" -DR_INTERPRETER="C:/Program Files/R/R-3.4.1/bin/R.exe" -DR_LIB="../r-3.4.1-import-lib/x64/Rlib" ../COPASI
ninja
```
