...
```{r}
stdvector <- CCopasiRootContainer_getFunctionList()$suitableFunctions(2, 2, "TriFalse")
stdvector$size()
```
...
```{r}
copasivector <- CCopasiRootContainer_getDatamodelList()
is(copasivector)
try(copasivector[0])
```
...
With functions that are called get in C already this seems to cause a bug.

I am considering to modify Copasi.R here too.
