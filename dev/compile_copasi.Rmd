...
The manylinux2014 container is the basis for the linux binaries.
...
```{bash, eval=FALSE}
cd copasi-source/
./d_build_manylinux2014_x64_clean.sh
cd ../
```

alternatives:
  on windows: d_build_manylinux2014_x64_clean.ps1
...
```{bash, eval=FALSE}
cd copasi-source/
./d_build_manylinux2014_x64_quick.sh
cd ../
```

alternatives:
  on windows: d_build_manylinux2014_x64_quick.ps1
...
```{r, eval=FALSE}
Sys.setenv(COPASI_LIB_PATH = normalizePath(file.path("..", "libs", .Platform$r_arch, paste0("COPASI", .Platform$dynlib.ext))))
install.packages("..", repos = NULL, type = "source", INSTALL_opts = "--no-multiarch --with-keep.source --install-tests")
```
