...
* (Internal) Remove workaround for removal of initialExpressions
* Fix setParameter
* Add model events
* Compile in a way that it can run on bioquant cluster
...
# Errors when loading repeatedly

```{r}
loadModel("data/ERKModel5.cps")
unloadAllModels()
loadModel("data/ERKModel5.cps")
```
