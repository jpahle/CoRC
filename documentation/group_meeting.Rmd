...
```{r, results='hide'}
model <- loadModel("data/brusselator_1.cps")
```

This will load the following model:

```{bash, eval=FALSE}
/Applications/COPASI/CopasiUI.app/Contents/MacOS/CopasiUI data/brusselator_1.cps &
```
...
species %>%
    ...
    invisible()
getSpecies()
saveCPS("data/brusselator_1_new.cps", overwrite = TRUE)
```
...
```{r, results='hide'}
model1 <- loadModel("data/brusselator_1.cps")
model2 <- loadModel("data/brusselator_2.cps")
```
...
```
```{bash, eval=FALSE}
/Applications/COPASI/CopasiUI.app/Contents/MacOS/CopasiUI data/brusselator_2.cps &
```

The models are idential but they have different options set for time-course simulation in the .cps file.

```{r}
tc1 <- runTimeCourse(100, 0.5, datamodel = model1)
tc2 <- runTimeCourse(100, 0.5, datamodel = model2)
tc1 %>%
    ggplot(aes(x = Time)) +
    geom_line(aes(y = X, color = "X")) +
    geom_line(aes(y = Y, color = "Y")) +
