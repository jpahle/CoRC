...
```{bash, eval=FALSE}
git clone --recursive https://github.com/jpahle/CoRC
```
...
```{r}
dyn.load(paste0("../development/copasi-source/build_copasi_r_bindings/copasi/bindings/R/COPASI", .Platform$dynlib.ext))
source("../development/copasi-source/build_copasi_r_bindings/copasi/bindings/R/COPASI.R")
cacheMetaData(1)
```
...
```{r, root.dir="../"}
setwd("../")
source("development/0-update-swig-in-package.R")
